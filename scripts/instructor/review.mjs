import {investigationComplete} from '../../src/core/onboarding/investigation.js';
...
export function reviewSubmission(mission,work){
 ...
 return {status:!current?'stale or not run':!result.runnable?'needs inputs':!result.passed?'revise':mission.investigation&&!investigationComplete(mission,work)?'observations needed':mission.codeCases?'code review required':reflectionChecks(work.reflection)&&(!mission.investigation||work.reflection.confirmedFor===work.attempt.id)?'checks passed; review reasoning':'reflection needed',checks:result.checks.filter(c=>!c.passed).map(c=>({message:c.message,actual:c.actual,expected:c.expected,error:c.error})),attempts:work.attempts?.length||0,answer:work.answer,reflection:work.reflection,at:work.attempt?.at,reportedCodeChecks:mission.codeCases&&Array.isArray(work.attempt?.result?.checks)?work.attempt.result.checks.slice(0,30):null,codeEvidence:mission.codeCases?'Student-browser results are not independent grading.':null};
}
